				...
				<div class="searchBox">
					<form method="get" action="<c:url value='/qna/view'/>">
						...
						<select name="keywordIn" id="keywordIn">
							...
							<option value="작성자"
								${paging.keywordIn == "작성자" ? "selected" : "" }>작성자</option>
				...
				<tbody>
					<c:forEach var="q" items="${qnaList }">
						<c:if test="${q.memberId eq loginMember.memberId}">
							<tr class="row"
								onclick="location.href='<c:url value="/qna/detail?no=${q.qnaId }"/>'">
								<td>${q.qnaId }</td>
								<td>${q.category }</td>
					...
					<div class="paging-pages">
						<c:choose>
							<c:when test="${ paging.prev }">
								<a
									href="<c:url value='/qna/view?nowPage=${paging.pageBarStart-1}&keyword=${paging.keyword }&keywordIn=${paging.keywordIn }'/>">
							...
							<c:otherwise>
								<a
									href="<c:url value='/qna/view?nowPage=${paging.pageBarStart-1}&keyword=${paging.keyword }&keywordIn=${paging.keywordIn }'/>"
									class="disabled"> <span class="material-symbols-outlined">chevron_left</span>
								</a>
							</c:otherwise>
						</c:choose>
						...
						<c:forEach var="i" begin="${paging.pageBarStart }"
							...
							<c:choose>
								<c:when test="${ i eq paging.nowPage }">
									<a
										href="<c:url value='/qna/view?nowPage=${i}&keyword=${paging.keyword }&keywordIn=${paging.keywordIn }'/>"
						...
						<c:choose>
							<c:when test="${ paging.next }">
								<a
									href="<c:url value='/qna/view?nowPage=${paging.pageBarEnd+1}&keyword=${paging.keyword }&keywordIn=${paging.keywordIn }'/>">
							...
							<c:otherwise>
								<a
									href="<c:url value='/qna/view?nowPage=${paging.pageBarEnd+1}&keyword=${paging.keyword }&keywordIn=${paging.keywordIn }'/>"
									class="disabled"> <span class="material-symbols-outlined">chevron_right</span>
								</a>
							</c:otherwise>
						</c:choose>
					</div>
